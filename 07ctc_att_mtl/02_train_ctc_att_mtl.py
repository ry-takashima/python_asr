...
if __name__ == "__main__":
    ...
    token_list_path = os.path.join(exp_dir, 'data', unit,
                                   'token_list')
    # 学習結果を出力するディレクトリ
    output_dir = os.path.join(exp_dir,
    ...
    # CTC損失関数を呼び出す．
    # blankは0番目と定義する．
    criterion_ctc = nn.CTCLoss(blank=0, reduction='sum')
    ...
    for epoch in range(max_num_epoch):
        ...
        for phase in ['train', 'validation']:
            ...
            for (features, labels, feat_lens,
                ...
                loss_att \
                    = criterion_att(outputs.view(b_size*t_size, 
                                    num_tokens),
                                    labels.contiguous().view(-1))
                # ラベルの平均長を掛ける
                loss_att *= np.mean(label_lens.numpy())

                # log_softmaxを通し、CTC損失値を計算する
                ctc_out = F.log_softmax(ctc_out, dim=2)
                loss_ctc = criterion_ctc(ctc_out.transpose(0, 1),
                                         labels,
                                         enc_lens,
                                         label_lens)
                # 処理した発話数で割る
                ...
                # 損失関数を重みづけ和する(マルチタスク学習)
                loss = (1.0 - mtl_weight)*loss_att \
                       + mtl_weight*loss_ctc
